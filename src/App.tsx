...
const EXECUTIVES = [
  {
    id: 1,
    name: "Bruno Duque",
    fields: ["bg", "bg_paytv", "bg_high", "bg_high_paytv"],
  },
  {
    id: 2,
    name: "Julia Masone",
...
const FIELD_LABELS: Record<string, string> = {
  ...
  bg_paytv: "BG Pay TV",
  bg_high: "BG High",
  bg_high_paytv: "BG High Pay TV",
  bg_digital: "BG Digital",
  bg_high_digital: "BG High Digital",
};
...
function getWeeks() {
  ...
  for (let i = -2; i <= 3; i++) {
    ...
    weeks.push({
      key: `w_${s.toISOString().slice(0, 10)}`,
      label: `${p(s)} - ${p(e)}`,
      month: mo,
    });
  }
...
const tdS = (r: boolean): any => ({
  padding: "8px 12px",
  fontSize: 13,
  textAlign: r ? ("right" as const) : ("left" as const),
  borderBottom: "1px solid #e8e4d0",
  whiteSpace: "nowrap" as const,
});
const numS = (v: number): any => ({
  ...tdS(true),
  color: v > 0 ? "#1a5c1a" : "#bbb",
...
export default function App() {
  ...
  useEffect(() => {
    ...
    onValue(ref(db, "cadence_env"), (snap) => {
      if (snap.exists()) setEnviado(snap.val());
    });
  }, []);

  useEffect(() => {
    setEnvInputs(enviado[dashWeek] || {});
  }, [dashWeek, enviado]);

  async function persistData(data: any) {
    try {
      await set(ref(db, "cadence_v2"), data);
    } catch (e) {}
  }
  async function persistEnviado(data: any) {
    try {
      await set(ref(db, "cadence_env"), data);
    } catch (e) {}
  }
  ...
  async function handleSaveStep2() {
    ...
    const upd = {
      ...allData,
      [key]: {
        ...(allData[key] || { exec: selectedExec, week: selectedWeek, values }),
        opps,
        ts: new Date().toISOString(),
      },
    };
  ...
  async function handleSaveEnviado() {
    ...
    setEnviado(upd);
    await persistEnviado(upd);
    setEnvSaved(true);
    setTimeout(() => setEnvSaved(false), 2500);
  }

  function updateOpp(id: any, field: string, val: any) {
    setOpps(opps.map((o: any) => (o.id === id ? { ...o, [field]: val } : o)));
  }
  function removeOpp(id: any) {
    setOpps(opps.filter((o: any) => o.id !== id));
  }
  function addOpp() {
    setOpps([...opps, emptyOpp()]);
  }
  ...
  const colTotal = (col: string) =>
    weekData.reduce((s, e) => s + parse((e.v as any)[col]), 0);
  const getEnv = (col: string) => parse(envInputs[col]);
  const allOpps = weekData.flatMap((e) =>
    (e.opps as any[])
      .filter((o: any) => o.client)
      .map((o: any) => ({ ...o, exec: e.name }))
  );
  const totalBG =
    colTotal("bg") + colTotal("bg_paytv") + colTotal("bg_digital");
  const totalBGHigh =
    colTotal("bg_high") +
    colTotal("bg_high_paytv") +
    colTotal("bg_high_digital");
  const exec = EXECUTIVES.find((e) => e.name === selectedExec);
  const wk = WEEKS.find((w) => w.key === dashWeek) || WEEKS[2];

  const EnvCell = ({ col }: { col: string }) => (
    <input
      value={envInputs[col] ? maskCurrency(envInputs[col]) : ""}
      onChange={(e) =>
        setEnvInputs({
          ...envInputs,
          [col]: e.target.value.replace(/[^\d]/g, ""),
        })
      }
      placeholder="—"
      style={{
      ...
      >
        <div
          style={{
            ...
            padding: "44px 40px",
            background: "#fff",
            border: "2px solid #c8b84a",
            borderRadius: 4,
            boxShadow: "6px 6px 0 #c8b84a33",
          }}
        >
          ...
          <button
            ...
            style={{
              width: "100%",
              padding: "13px",
              background: selectedExec ? "#4a7c3f" : "#ccc",
              border: "none",
              borderRadius: 3,
              color: "#fff",
              fontSize: 14,
              fontWeight: 700,
              cursor: selectedExec ? "pointer" : "default",
              letterSpacing: 1,
              ...T,
            }}
          >
          ...
          <div key={s.n} style={{ display: "flex", alignItems: "center" }}>
            <div
              style={{
                ...
                borderRadius: 3,
                background:
                  step === s.n ? "#4a7c3f" : step > s.n ? "#d4ecd4" : "#f0ede0",
                border: `1.5px solid ${
                  step === s.n ? "#4a7c3f" : step > s.n ? "#4a7c3f" : "#ddd"
                }`,
              }}
            >
              <span
                style={{
                  ...
                  borderRadius: "50%",
                  background:
                    step === s.n ? "#fff" : step > s.n ? "#4a7c3f" : "#bbb",
                  color: step === s.n ? "#4a7c3f" : "#fff",
                  fontSize: 11,
                  fontWeight: 700,
                  display: "flex",
                  alignItems: "center",
              ...
              <span
                style={{
                  ...
                  fontWeight: 700,
                  color:
                    step === s.n ? "#fff" : step > s.n ? "#2a5c1a" : "#999",
                }}
              >
                {s.label}
              </span>
                ...
                {exec?.fields.map((field, i) => (
                  ...
                  >
                    ...
                    <div
                      style={{
                        padding: "8px 16px",
                        borderLeft: "1px solid #f0ede0",
                        background: i % 2 === 0 ? "#fafaf5" : "#fff",
                      }}
                    >
                      <input
                        value={values[field] ? maskCurrency(values[field]) : ""}
                        onChange={(e) =>
                          setValues({
                            ...values,
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                >
                  <div>
                    ...
                    <span
                      style={{ marginLeft: 10, fontSize: 12, color: "#888" }}
                    >
                      Adicione as oportunidades relevantes da semana
                    </span>
                  </div>
                  <button
                    onClick={addOpp}
                ...
                {opps.map((opp: any, i: number) => (
                  <div
                    ...
                    style={{
                      background: "#fff",
                      border: `2px solid ${
                        PLATFORM_COLOR[opp.platform]?.border || "#ddd"
                      }`,
                      borderRadius: 4,
                      marginBottom: 12,
                      overflow: "hidden",
                  ...
                  >
                    <div
                      style={{
                        background:
                          PLATFORM_COLOR[opp.platform]?.bg || "#fafafa",
                        padding: "8px 16px",
                        display: "flex",
                        justifyContent: "space-between",
                        alignItems: "center",
                    ...
                    >
                      <div style={{ display: "flex", gap: 6 }}>
                        {PLATFORMS.map((p) => (
                          <button
                            key={p}
                            onClick={() => updateOpp(opp.id, "platform", p)}
                            style={{
                              padding: "4px 12px",
                              borderRadius: 20,
                              border: `1.5px solid ${
                                p === opp.platform
                                  ? PLATFORM_COLOR[p].border
                                  : "#ddd"
                              }`,
                              background:
                                p === opp.platform
              ...
              <div style={{ display: "flex", gap: 10 }}>
                ...
                <button
                  ...
                  style={{
                    flex: 1,
                    padding: "14px",
                    background: saved ? "#4a7c3f" : "#1a2e1a",
                    border: "none",
                    borderRadius: 3,
                    color: "#fff",
                    fontSize: 14,
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        <div
          style={{
            display: "flex",
            marginBottom: 20,
            borderBottom: "2px solid #e8e4d0",
          }}
        >
          {[
            { key: "forecast", label: "📊 Previsão BG" },
            { key: "opps", label: "⭐ Principais Oportunidades" },
          ].map((tab) => (
            <button
              key={tab.key}
              ...
              style={{
                ...
                color: dashTab === tab.key ? "#1a2e1a" : "#888",
          ...
          <>
            ...
            >
              {[
                {
                  title: "PREVISÃO",
                  cols: ["bg", "bg_paytv", "bg_digital"],
                  heads: ["BG", "Pay TV", "Digital"],
                },
                {
                  title: "PREVISÃO HIGH",
                  cols: ["bg_high", "bg_high_paytv", "bg_high_digital"],
                  heads: ["BG HIGH", "PayTV HIGH", "Digital HIGH"],
                },
              ].map((panel) => (
                <div
                  key={panel.title}
                  ...
                  >
                    ...
                    <tfoot>
                      ...
                      <tr style={{ background: "#f0ede0" }}>
                        ...
                        >
                          {fmtFull(
                            panel.cols.reduce((s, c) => s + colTotal(c), 0)
                          )}
                        </td>
                      </tr>
                      <tr style={{ background: "#1a2e1a" }}>
                        ...
                        {panel.cols.map((c) => (
                          <td key={c} style={{ padding: "5px 8px" }}>
                            <EnvCell col={c} />
                          </td>
                        ))}
                      </tr>
                      <tr style={{ background: "#0f1f0f" }}>
                        <td colSpan={4} style={{ padding: "8px 10px" }}>
                          <button
                            onClick={handleSaveEnviado}
                            style={{
                              width: "100%",
                              padding: "7px",
                              background: envSaved ? "#4a7c3f" : "#c8b84a",
                              border: "none",
                              borderRadius: 2,
                              color: envSaved ? "#fff" : "#1a2e1a",
                              fontSize: 12,
                              fontWeight: 700,
                              cursor: "pointer",
                              ...T,
            ...
            >
              {EXECUTIVES.map((ex) => {
                ...
                const hasOpp = !!(
                  entry?.opps?.length && entry.opps.some((o: any) => o.client)
                );
                return (
                  <div
                    key={ex.id}
                    style={{
                      padding: "9px 14px",
                      background: hasBG ? "#f0f8f0" : "#fff",
                      border: `1.5px solid ${hasBG ? "#4a7c3f" : "#ddd"}`,
                      borderRadius: 3,
                    }}
                  >
                    <div
                    ...
                    >
                      ...
                      <span
                        style={{
                          fontSize: 12,
                          color: hasBG ? "#2a5c1a" : "#999",
                          fontWeight: hasBG ? 700 : 400,
                        }}
                      >
                    ...
                    <div
                      style={{
                        fontSize: 10,
                        color: hasOpp ? "#4a7c3f" : "#bbb",
                        paddingLeft: 22,
                      }}
                    >
                      {hasOpp
                        ? `⭐ ${
                            entry.opps.filter((o: any) => o.client).length
                          } oportunidade(s)`
                        : "Sem oportunidades"}
                    </div>
                  </div>
            ...
            {allOpps.length === 0 ? (
              ...
              >
                ...
                <p style={{ margin: 0, fontSize: 14 }}>
                  Nenhuma oportunidade registrada para esta semana.
                </p>
              </div>
            ) : (
              <div
                        ...
                        >
                          ...
                          <td
                            style={{
                              ...
                              fontSize: 13,
                              fontWeight: 700,
                              color: parse(o.value) > 0 ? "#1a5c1a" : "#bbb",
                              textAlign: "right" as const,
                              whiteSpace: "nowrap" as const,
                            }}
                          >
